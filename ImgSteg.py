...
⣿⣿⣿⠁ ⣠⣾⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿
⣿⡿⠻⣷⣤⣾⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿’'''
 art2 = '''Hide Your Secret Message!'''
 def main(self,root):
        root.title('ImageSteganography')
        root.geometry('500x600')
        root.resizable(width =False, height=False)
        f = Frame(root, bg='DodgerBlue4', relief=GROOVE)
        f.place(x=10, y=80)
        
        title = Label(f,text='Image Steganography')
        title.config(font=('Bell MT', '32', 'bold '))
        title.configure(bg='DodgerBlue4')
        title.config(fg='lemon chiffon')
        title.grid(row=1, pady=30)
        ...
        ascii_art.config(font=('courier',5))
        ascii_art.configure(bg='DodgerBlue', fg='alice blue')

        ascii_art2 = Label(f,text=self.art2)
        # ascii_art.config(font=('MingLiU-ExtB',50))
        ascii_art2.config(font=('courier',12,'bold'))
        ascii_art2.configure(bg='DodgerBlue4', fg='lemon chiffon')
